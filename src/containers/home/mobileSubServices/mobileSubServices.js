...
export default function MobileSubServices() {
    ...
    if (icon) {
      ...
      axios(config)
        .then((response) => {
          ...
          var config2 = {
            method: 'post',
            url: `${process.env.REACT_APP_BACKEND_URL}/api/mobileSubService`,
            // url: 'http://localhost:5000/api/mobileSubService',
            data: obj
          };
          axios(config2)
            .then((res) => {
    ...
    if (icon) {
      ...
      axios(config)
        .then((response) => {
          ...
          var config2 = {
            method: 'put',
            url: `${process.env.REACT_APP_BACKEND_URL}/api/mobileSubService/${editObj._id}`,
            // url: `http://localhost:5000/api/mobileSubService/${editObj._id}`,
            data: obj
          };
          axios(config2)
            .then((res) => {
  ...
  return (
    <>
      <div className='d-flex justify-content-between w-100'>
        <h3>Mobile Sub Services</h3>
        {activeUser && activeUser.permissions[0].subServiceAdd &&
          <Button type="primary" onClick={showModal}>Add Mobile Sub Service</Button>
        }
      </div>
      <Table pagination={{ pageSize: 5 }} columns={columns} dataSource={subServices} />

      <Modal
        title={editObj ? `Edit ${editObj.name}` : "Add Mobile Sub Service"}
        open={isModalOpen}
        onOk={handleOk}
